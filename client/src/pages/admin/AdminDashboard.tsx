    ...
    if (chartPeriod === "day") {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        if (quarterMonths.length > 0) {
          ...
          const quarterOrders = quarterMonths.reduce(
            (sum, m) => sum + m.orders,
            0
          );
          quarterData.push({
  ...
  const weekData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayData = stats.weeklyRevenue.find(
        (d) =>
          d._id.day === date.getDate() && d._id.month === date.getMonth() + 1
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Отслеживайте показатели эффективности вашего бизнеса.
              {realtimeStats && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <MetricCard
          ...
          subtitle={`Сегодня: ${formatCurrency(stats?.todayRevenue || 0)}`}
        ...
        <MetricCard
          ...
          subtitle={`Сегодня: ${stats?.todayOrders || 0} | В обработке: ${stats?.pendingOrders || 0}`}
          color="text-blue-600"
          bgColor="bg-blue-50"
          loading={refreshing}
        />
        <MetricCard
          ...
          subtitle={`Новые: ${stats?.newCustomersThisMonth || 0} | Активные: ${stats?.activeUsers || 0}`}
          ...
          subtitle={
            stats?.lowStockCount && stats?.lowStockCount > 0 ? (
              <span className="text-orange-600 font-medium">
                {stats.lowStockCount} заканчиваются
              </span>
            ) : (
              <span>В наличии</span>
            )
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-600">Новый заказы:</span>
                <span className="text-sm font-bold text-blue-600">
                  {realtimeStats.recentOrders}
                </span>
              </div>
              <div className="flex items-center gap-2">
              ...
              <AreaChart data={chartData}>
                ...
                <Tooltip
                  ...
                  formatter={(value: any, name: string) => {
                    if (name === "Доход") return formatFullCurrency(value);
                    return value;
                  }}
                />
                <Area
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className={`${bgColor} p-3 rounded-lg ${color}`}>{icon}</div>
      </div>
    </div>
  );
};
