...
router.get("/:id/qr-code", protect, async (req, res) => {
  try {
    ...
    if (order.paymentMethod !== "BankTransfer") {
      return res
        .status(400)
        .json({ message: "QR code only available for bank transfer payments" });
    }
  ...
  try {
    ...
    if (previousStatus !== "delivered" && orderStatus === "delivered") {
      ...
      if (user) {
        ...
        // Calculate totals
        const totalSpent = allDeliveredOrders.reduce(
        ...
        console.log(
          `✅ Updated analytics for user ${user._id}: totalSpent=${totalSpent}, totalOrders=${totalOrders}`
        );
      }
    }
...
router.put("/:id/confirm-payment", protect, async (req, res) => {
  try {
    ...
    if (order.paymentMethod !== "BankTransfer") {
      return res
        .status(400)
        .json({ message: "This order does not use bank transfer" });
    }
...
cron.schedule("0 * * * *", async () => {
  try {
    ...
    if (expiredCount > 0) {
      console.log(
        `[CRON] Expired ${expiredCount} overdue bank transfer orders`
      );
    }
  } catch (error) {
