...
function buildRulesFromProduct(product) {
  return {
    styles: product.style || [],
    seasons: product.season || [],
    brands: [product.brand],
    // colorGroups: product.colorGroup || [],
  ...
  try {
    ...
    for (const anchor of anchors) {
      const capsule = new Capsule({
        name: `Капсула с ${anchor.name}`,
        type: "auto",
        isPublished: false,
        rules: buildRulesFromProduct(anchor),
      });
    ...
    res.json({
      message: "Capsules generated",
      count: createdCapsules.length,
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: error.message });
