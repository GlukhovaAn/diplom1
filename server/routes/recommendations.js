...
class RecommendationCache {
  ...
  get(key, isPersonalized = false) {
    ...
    const duration = isPersonalized
      ? config.cache.personalizedDuration
...
const getDynamicDistribution = async (userId, limit) => {
  try {
    ...
    const recentViews =
      user.viewHistory?.filter(
        (v) =>
          new Date(v.viewedAt) >
          new Date(Date.now() - config.limits.recentActivityWindow)
      ).length || 0;

    const recentSearches =
      user.searchHistory?.filter(
        (s) =>
          new Date(s.searchedAt) >
          new Date(Date.now() - config.limits.recentActivityWindow)
      ).length || 0;
...
async function getCollaborativeRecommendations(userId, limit = 20) {
  ...
  try {
    const [userOrders, userInteractions] = await Promise.all([
      ...
      User.findById(userId).select("viewHistory interactions.wishlist").lean(),
    ]);
    ...
    userOrders.forEach((order) => {
      const weight = calculateTimeDecay(
        order.createdAt,
        config.timeDecay.purchaseDecayDays
      );
      order.items.forEach((item) => {
        if (item.product) {
          ...
          userProducts.set(
            productId,
            (userProducts.get(productId) || 0) +
              weight * config.weights.purchase * (item.quantity || 1)
          );
        }
      });
    });
    ...
    if (userInteractions?.viewHistory) {
      userInteractions.viewHistory
        ...
        .forEach((view) => {
          ...
          const weight = calculateTimeDecay(
            view.viewedAt,
            config.timeDecay.viewDecayDays
          );
          userProducts.set(
            productId,
    ...
    const productIds = Array.from(userProducts.keys()).map(
  ...
  try {
    ...
    if (user.viewHistory && user.viewHistory.length > 0) {
      ...
      viewedProducts.forEach((product, index) => {
        ...
        product.colors?.forEach((color) => {
          preferences.colors.set(
            color,
            (preferences.colors.get(color) || 0) + baseWeight * 0.5
          );
        });

        product.sizes?.forEach((size) => {
          preferences.sizes.set(
            size,
            (preferences.sizes.get(size) || 0) + baseWeight * 0.5
          );
        });
        ...
        product.tags?.forEach((tag) => {
          preferences.tags.set(
            tag,
            (preferences.tags.get(tag) || 0) + baseWeight * 0.7
          );
        });
      });
    }
      ...
      recentSearches.forEach((search, index) => {
        ...
        words.forEach((word) => {
          if (word.length > 2) {
            preferences.tags.set(
              word,
              (preferences.tags.get(word) || 0) + weight
            );
          }
        });
      });
        ...
        if (item.product) {
          if (item.product.category) {
            preferences.categories.set(
              ...
              (preferences.categories.get(item.product.category) || 0) +
                orderWeight
            );
          }
          ...
          item.product.tags?.forEach((tag) => {
            preferences.tags.set(
              tag,
              (preferences.tags.get(tag) || 0) + orderWeight * 0.8
            );
          });
    ...
    if (user.interactions?.wishlist) {
      const wishlistProducts = await Product.find({
        _id: {
          $in: user.interactions.wishlist.map((w) => w.product).filter(Boolean),
        },
      }).lean();
      ...
      wishlistProducts.forEach((product) => {
        if (product.category) {
          preferences.categories.set(
            product.category,
            (preferences.categories.get(product.category) || 0) +
              config.weights.wishlist
          );
        }
        if (product.brand) {
          preferences.brands.set(
            product.brand,
            (preferences.brands.get(product.brand) || 0) +
              config.weights.wishlist
          );
        }
...
async function getTrendingProducts(limit = 20) {
  ...
  try {
          ...
          recentOrders: {
            $sum: {
              $cond: [
                ...
                3,
                {
                  $cond: [{ $gte: ["$createdAt", threeDaysAgo] }, 2, 1],
                },
              ],
            },
          },
    ...
    const viewTrending = await User.aggregate([
      ...
      },
      {
        $group: {
          ...
          recentViews: {
            $sum: {
              $cond: [{ $gte: ["$viewHistory.viewedAt", oneDayAgo] }, 2, 1],
            },
          },
    ...
    for (const productId of allProductIds) {
      ...
      const viewData = viewMap.get(productId) || {
        viewerCount: 0,
        viewCount: 0,
        recentViews: 0,
        avgDuration: 0,
      };
      const wishlistData = wishlistMap.get(productId) || {
        wishlistUserCount: 0,
        wishlistCount: 0,
      };
      const cartData = cartMap.get(productId) || {
        cartUserCount: 0,
        cartCount: 0,
      };
      ...
      const totalUniqueUsers = Math.max(
        ...
        orderData.buyerCount +
          viewData.viewerCount -
          Math.floor(orderData.buyerCount * 0.3), // Assume 30% overlap
  ...
  try {
    ...
    if (preferences) {
      ...
      if (preferences.categories.length > 0) {
        conditions.push({
          category: { $in: preferences.categories.slice(0, 3) },
        });
      }
  ...
  try {
    ...
    if (user.viewHistory && user.viewHistory.length > 0) {
      ...
      if (recentProductIds.length > 0) {
        const viewedProducts = await Product.find({
          ...
          .select("category brand tags price")
          .lean();
    ...
    if (prices.length > 0) {
      ...
      const stdDev = Math.sqrt(
        prices.reduce((sq, n) => sq + Math.pow(n - avgPrice, 2), 0) /
          prices.length
      );
      priceRange = {
...
router.get("/", optionalAuth, async (req, res) => {
  try {
    ...
    const { type = "mixed", limit = config.limits.defaultRecommendations } =
      req.query;
    const safeLimit = Math.min(
      Number(limit) || config.limits.defaultRecommendations,
      config.limits.maxRecommendations
    );
    ...
    switch (type) {
      ...
      default: {
        ...
        const user = await User.findById(userId)
          .select("viewHistory analytics")
          .lean();
        const isActiveUser =
          (user?.viewHistory?.length || 0) > config.tracking.viewCountThreshold;
...
router.get("/product/:productId", optionalAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      complementary: Array.isArray(response.complementary)
        ? response.complementary
        : [],
      userRecommended: Array.isArray(response.userRecommended)
        ? response.userRecommended
        : [],
    });
  } catch (error) {
...
router.post("/track", protect, async (req, res) => {
  try {
    ...
    if (
      ["view", "addToCart", "wishlist", "click"].includes(action) &&
      !productId
    ) {
      return res
        .status(400)
        .json({ message: "Product ID is required for this action" });
    }
    ...
    switch (action) {
      ...
      case "click":
        if (productId && metadata.recommendationType) {
          console.log(
            `Recommendation click: ${metadata.recommendationType} -> ${productId} by user ${userId}`
          );
        }
        break;
      ...
      case "recommendation_load":
        console.log(
          `Recommendations loaded: ${metadata.type} - ${metadata.count} items`
        );
        break;

      case "scroll":
        console.log(
          `Scroll event: ${metadata.direction} from ${metadata.fromIndex} to ${metadata.toIndex}`
        );
        break;
    }
    ...
    orders.forEach((order) => {
      order.items.forEach((item) => {
        if (item.product?.category) {
          ...
          const totalSpent =
            (categoryCount.get(cat)?.totalSpent || 0) +
      ...
      recommendationQuality: {
        personalizedAvailable:
          (user?.viewHistory?.length || 0) >
            config.tracking.viewCountThreshold ||
          (user?.analytics?.totalOrders || 0) > 0,
...
setInterval(
  () => {
    ...
    if (cleaned > 0) {
      console.log(
        `[Cache Cleanup] Removed ${cleaned} expired entries at ${new Date().toISOString()}`
      );
    }
  },
  5 * 60 * 1000
...
setInterval(() => {
  recommendationCache.invalidatePattern("trending");
  console.log(
    "[Trending Cache] Invalidated for fresh data at",
